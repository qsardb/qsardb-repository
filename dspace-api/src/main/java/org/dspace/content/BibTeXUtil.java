...
public class BibTeXUtil {
	...
	public List<Key> getFields(Key type){
		...
		if(TYPE_MISC.equals(type)) {
			return Arrays.asList(KEY_AUTHOR, KEY_TITLE, KEY_HOWPUBLISHED, KEY_YEAR, KEY_URL, KEY_DOI);
		} else

		if(TYPE_TECHREPORT.equals(type)) {
			return Arrays.asList(KEY_AUTHOR, KEY_TITLE, KEY_NUMBER, KEY_INSTITUTION, KEY_ADDRESS, KEY_YEAR, KEY_MONTH, KEY_URL, KEY_DOI);
		}
